...
class AiSEO extends Controller
{
    public function get(){
        return "jaaja";
    }
    ...
    public function bulk(){
        ...
        $files = //json_decode(File::get(public_path('niches.json')), true);
            Artisan::command('run-gpt', function () use ($files) {
//sleep(10);
            ...
            collect($title_list)->each(function ($titles, $category_name) use ($niche_name, $client) {
                ...
                if($titles){
                    for ($i = 0; count($titles) > $i; $i++) {
                        $title = $titles[$i];
                        $url = "https://digest.ng/?rest_route=/gpt-post-maker/v1/post_exists/";
...
p. Pin this article for future reference and keep practicing! You'll soon become a pro with these tutorials.\"";
        ...
        function create_post_on_wp($title, $category, $niche_name, $content, $client) {
            ...
            $url = "https://digest.ng/?rest_route=/gpt-post-maker/v1/create_post/";
            //dd($url);
       ...
       // dd($message);
        ...
        $response = $client->request('POST', 'https://api.openai.com/v1/chat/completions', [
            ...
            'json' => [
                ...
                "messages" => [["role" => "user", "content" => "You are a wickedly funny, and hilarious Nigerian news writer with an uncanny sense of humor. In  Nigerian pidgin, totally rewrite this piece of news and also suggest a befitting title: $message"]]
            ]
        ]);
        ...
        // Extract the generated blog post titles
        $blog_post_titles = [];

        foreach (json_decode($response_data->choices[0]->message->content) as $line) {
            // Remove the number at the beginning of each line
            $blog_post_titles[] = $line;
        }

        // Return the blog post titles in a JSON array
        return $blog_post_titles;
    ...
    public function generate_titles(){
       ...
       // dd($message);
        ...
        $response = $client->request('POST', 'https://api.openai.com/v1/chat/completions', [
            ...
            'json' => [
                "model" => "gpt-3.5-turbo",
                "messages" => [["role" => "user", "content" => "I am writing an anabolic focused blog. Give me 500 unique blog posts titles on '$message'. Put the list in a json array. Remove the number at the beginning of each line"]]
            ]
        ]);
